...
<body>
	<div class="container">
	<%if(dto.getPrevNum()!=0){ %>
		<a href="detail.jsp?num=<%=dto.getPrevNum() %>
		&keyword=<%=encodedK%>&condition=<%=condition%>">prev</a>
	<%} %>	
	<%if(dto.getNextNum()!=0){ %>
		<a href="detail.jsp?num=<%=dto.getNextNum() %>
		&keyword=<%=encodedK%>&condition=<%=condition%>">next</a>
	<%} %>
	<%if(!keyword.equals("")){ %>
		<p>
			Searched result with <strong><%=condition%></strong> condition, 
			and <strong><%=keyword%></strong> keyword.
		</p>
	<%} %>
		...
		<ul>
			...
			<%if(dto.getWriter().equals(id)){ %>
				...
				<li>
					<a href="private/delete.jsp?num=<%=dto.getNum()%>">
					delete</a>
				</li>
			...
			<ul>
				<%for(CafeCommentDto tmp:commentList){ %>
					<%if(tmp.getDeleted().equals("yes")){ %>
						<li>Deleted comment.</li>
					<%
						//continue; 아래의 코드를 수행하지 않고 for문으로 실행순서 다시 보내기
						continue;
					} %>
					...
					<%} %>
						...
						<dl>
							...
							<%}else{ %>
								...
								<%=tmp.getProfile() %>" />
							<%} %>
							<a data-num="<%=tmp.getNum()%>" 
							href="javascript:" class="reply-link">reply</a>
							<%if(id!=null && tmp.getWriter().equals(id)){ %>
								<a data-num="<%=tmp.getNum()%>" 
								class="delete-link" href="javascript:">
								delete</a>
								<a data-num="<%=tmp.getNum()%>" 
								class="update-link" href="javascript:">
								modify</a>
							<%} %>
							</dt>
							<dd>
						...
						<form id="updateForm<%=tmp.getNum()%>" class="update-form comment-form" action="private/comment_update.jsp" method="post">
		...
		<form class="comment-form insert-form" action="private/comment_insert.jsp" method="post">
			...
			<textarea name="content" cols="30" rows="10"><%if(!isLogin){%>Please log in.<%}%></textarea>
			<button type="submit">submit</button>
		</form>
	</div>
...
<script>
	...
	/*
		detail.jsp페이지 로딩 시점에 만들어진 1페이지에 해당하는 댓글에 
		이벤트리스너 등록하기
	*/
   ...
   */
	window.addEventListener("scroll", function(){
		...
		if(isBottom && !isLoading && !isLast){
			...
			*/
			...
			//로딩이 끝났다고 표시한다.
			isLoading=false;
			//새로 추가된 댓글 li 요소 안에 있는 a 요소를 찾아서 이벤트리스너 등록하기
			addUpdateListener(".page-"+currentPage+" .update-link");
			addDeleteListener(".page-"+currentPage+" .delete-link");
			addReplyListener(".page-"+currentPage+" .reply-link");
			//새로 추가된 댓글 li 요소 안에 있는 댓글 수정 폼에 이벤트리스너 등록하기
	...
	//인자로 전달되는 선택자를 이용해서 이벤트리스너를 등록하는 함수
	function addUpdateListener(sel){
		...
		for(let i=0; i<updateLinks.length; i++){
			updateLinks[i].addEventListener("click",function(){
				//click 이벤트가 일어난 바로 그 요소의 data-num속성의 value값을 읽어온다. 
	...
	function addDeleteListener(sel){
		...
		for(let i=0; i<deleteLinks.length; i++){
			deleteLinks[i].addEventListener("click",function(){
				//click 이벤트가 일어난 바로 그 요소의 data-num속성의 value값을 읽어온다. 
				const num=this.getAttribute("data-num"); //댓글의 글번호
				const isDelete=confirm("Are you sure you want to delete your comment?");
				if(isDelete){
					...
					.then(function(data){
						//만일 삭제 성공이면
						if(data.isSuccess){
							//댓글이 있는 곳에 삭제된 댓글입니다 를 출력해 준다.
							document.querySelector("#reli"+num).innerText="Deleted Comment!";
						}
					})
				}
			});
	...
	function addReplyListener(sel){
		...
		//반복문 돌면서 모든 링크에 이벤트리스너 함수 등록하기
		for(let i=0; i<replyLinks.length; i++){
			replyLinks[i].addEventListener("click",function(){
	            if(!isLogin){
	                const isMove=confirm("You must be logged in to reply. Would you go to the login page?");
	                if(isMove){
	                   location.href=
	                      "${pageContext.request.contextPath}/users/loginform.jsp?url=${pageContext.request.contextPath}/cafe/detail.jsp?num=<%=num%>";
	                }
				...
				//현재 문자열을 읽어온다 ("reply" or "cancel")
				let current=this.innerText;
				if(current=="reply"){
					//번호를 이용해서 댓글의 댓글 폼을 선택해서 보이게 한다. 
					...
					//번호를 이용해서 댓글의 댓글 폼을 선택해서 안 보이게 한다. 
	...
	function addUpdateFormListener(sel){
		...
		for(let i=0; i<updateForms.length; i++){
			...
			updateForms[i].addEventListener("submit",function(e){
				//submit 이벤트가 일어난 form의 참조값을 form이라는 변수에 담기
				const form=this;
				//폼 제출을 막은 다음
				e.preventDefault();
				...
				.then(function(data){
					if(data.isSuccess){
						/*
							...
							특정 문서의 참조값.querySelector()는 해당 문서 객체의 자손요소 중에서 
							특정 요소의 참조값을 찾는 기능
						*/
						const num=form.querySelector("input[name=num]").value;
						const content=form.querySelector("textarea[name=content]").value;
						//수정폼에 입력한 value값을 pre 요소에도 출력하기
