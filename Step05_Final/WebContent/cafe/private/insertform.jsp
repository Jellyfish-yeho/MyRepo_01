...
<body>
	...
	2. WebContent 에 upload 폴더 만들어 두기 
		=> 이미지를 선택하면 자동으로 ajax로 업로드, 저장된 파일명으로 img 마크업 생성하는 것 
			(파일명에는 다른 이미지와 겹치지 않도록 긴 문자열도 같이 들어간다)
	3. WebContent/WEB-INF/lib 폴더에 
		commons-io.jar 파일과 commons-fileupload.jar 파일 붙여 넣기
	4. <textarea id="content" name="content"> 
		content가 아래의 javascript 에서 사용되기 때문에,
		다른 이름으로 바꾸고 싶으면 javascript 에서  content 를 찾아서 모두 다른 이름으로 바꿔주면 된다. 
...
<script>
	...
	document.querySelector("#insertForm")
		...
		//만일 폼 제출을 막고 싶으면  e.preventDefault();을 수행하게 해서 폼 제출을 막아준다.
		if(title.length < 5){
