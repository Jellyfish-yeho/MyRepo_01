...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" %>
<%
	//get방식 파라미터로 전달되는 파일번호 읽어오기
	int num=Integer.parseInt(request.getParameter("num"));
	//db에서 다운로드해줄 파일의 정보를 얻어온다.
   ...
   //File.separator는 window에서는 \, linux에서는 /를 참조하게 된다. 
   ...
   //response.getOutputStream() 메소드는 클라이언트에게 출력할 수 있는 OutputStream 객체를 반환한다. 
   ...
   while(true){
      ...
      bos.flush(); //출력
      //bufferedOutputStream은 response를 통해 얻어왔기 때문에 close하면 response가 닫혀버린다.
      //웹사이트가 응답하면서 알아서 닫히게 된다 (?)
   }
