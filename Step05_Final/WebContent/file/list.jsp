...
<%
	...
	/*
		[검색 키워드에 관련된 처리]
		- 검색 키워드가 파라미터로 넘어올 수도 있고, 안 넘어올 수도 있다.
	*/
	...
	if(keyword==null){
		...
		//클라이언트 웹브라우저에 출력할 때 "null"을 출력되지 않게 하기 위해 
		keyword="";
		condition="";
	}
	...
	//FileDto 객체에 startRowNum, endRowNum을 담는다.
   ...
   //로그인된 아이디 (로그인하지 않은 경우 null이므로 주의!!!!)
...
<body>
	...
	<a href="private/upload_form.jsp">upload file</a>
		<h1>Library List</h1>
		<table>
			<thead>
				<tr>
					<th>no.</th>
					<th>writer(uploader)</th>
					<th>title(description)</th>
					<th>file name(original file name)</th>
					<th>file size</th>
					<th>regdate</th>
					<th>delete</th>
				</tr>
		...
		<div style="clear:both;"></div>
			<form action="list.jsp" method="get">
				...
				<select name="condition" id="condition">
					<option value="title_filename" <%=condition.equals("title_filename")?"selected":""%>>title & fileName</option>
				...
				<input type="text" name="keyword" placeholder="enter word..." value="<%=keyword%>"/>
				<button type="submit">search</button>
			</form>	
		<%if(!condition.equals("")){ %>
			<p>
				<strong><%=totalRow%></strong> posts searched. 
			</p>
		<%} %>
	</div>			
	<script>
		function deleteConfirm(num){
			const isDelete=confirm("Are you sure to delete "+num+" file?");
			if(isDelete){
