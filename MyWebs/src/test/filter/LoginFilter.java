 ...
 *  = => %3D
 *  
 *  /users/loginform.jsp?url=%2Ftest%2Fxxx.jsp%3Fa=xxx&b%3Dxxx
 * 
 */
/*
 ...
 * 
 */
@WebFilter(urlPatterns = {"/users/private/*","/message/private/*"})
public class LoginFilter implements Filter{
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		//1. 로그인을 했는지 로그인된 아이디를 읽어온다. (HttpSession 객체)
			HttpServletRequest req = (HttpServletRequest) request;
			HttpSession session=req.getSession();
			String id=(String)session.getAttribute("id");
			if(id!=null) {//2. 만일 로그인을 했다면 관여하지 않고 요청의 흐름을 이어간다. (FilterChain 객체)
				...
				//3. 로그인을 하지 않았다면 /users/loginform.jsp 페이지로 리다일렉트 이동 시킨다. (HttpServletResponse 객체) 
